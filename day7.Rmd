...
We can try to carry out kriging interpolation on this dataset, but will see that it generates errors, mentioning singular covariance matrices. A common cause of this is that we have observation locations with duplicate observations.
...
plot(kr[1], reset = FALSE, main = "bd kriging prediction")
plot(st_geometry(roi), add = TRUE, col = NA, border = 'orange')
kr$se = sqrt(kr$var1.var)
plot(kr["se"], reset = FALSE, breaks = "equal", main = "bd kriging standard error")
plot(st_geometry(roi), add = TRUE, col = NA, border = 'orange')
...
We can create a finer (long/lat) grid, e.g. by
